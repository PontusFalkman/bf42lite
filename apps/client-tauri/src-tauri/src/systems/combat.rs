...
use crate::config::{GameConfig, WeaponConfig};
use crate::player::{Player, RESPAWN_TIME};
use crate::protocol::ClientMessage;

/// Resolve the primary weapon for a given player from the game config.
fn weapon_for_player<'a>(
    player: &Player,
    config: &'a GameConfig,
) -> Option<&'a WeaponConfig> {
    // We assume Player has a `class_id: u8` that matches ClassConfig.id
    let class_cfg = config.classes.iter().find(|c| c.id == player.class_id)?;
    config
        .weapons
        .iter()
        .find(|w| w.id == class_cfg.primary_weapon_id)
}

/// Server-side combat update:
/// - handles fire cooldowns and respawns
/// - performs hitscan
/// - applies damage using weapon/class config
pub fn update(
    ...
    // Snapshot shooter data so we don't fight the borrow checker later
    let shooter_data: Vec<(u32, f32, f32, f32, f32, f32, bool)> = players
        .iter()
        .map(|(id, p)| {
            (
                *id,
    ...
    for (shooter_id, sx, sy, sz, syaw, spitch, sdead) in &shooter_data {
        ...
        if let Some(msg) = input_map.get(shooter_id) {
            ...
            if fire && current_cooldown <= 0.0 {
                // Placeholder entry; we may replace victim_id below
                ...
                for (victim_id, victim) in players.iter() {
                    if shooter_id == victim_id || victim.is_dead {
                        continue;
                    }
                    ...
                    let dist_sq =
                        (oc_x * oc_x + oc_y * oc_y + oc_z * oc_z) - (projection * projection);
                ...
                if let Some(vid) = hit_victim {
                    // Replace the placeholder (shooter, 0) with a real victim
                    hits.pop();
    ...
    // 3. Apply Damage (data-driven via GameConfig)
    let mut kills_to_award: Vec<u32> = Vec::new();

    for (shooter_id, victim_id) in hits {
        // Set fire cooldown based on weapon config (even on a miss: victim_id == 0)
        if let Some(shooter) = players.get_mut(&shooter_id) {
            if let Some(weapon_cfg) = weapon_for_player(shooter, config) {
                shooter.fire_cooldown = weapon_cfg.fire_rate.max(0.01);
            } else {
                // Safe fallback if config is missing/misconfigured
                shooter.fire_cooldown = 0.15;
            }
        }

        if victim_id != 0 {
            // 1) Compute damage using an immutable borrow
            let damage: f32 = if let Some(shooter_for_damage) = players.get(&shooter_id) {
                if let Some(weapon_cfg) = weapon_for_player(shooter_for_damage, config) {
                    weapon_cfg.damage_per_hit
                } else {
                    34.0 // fallback damage
                }
            } else {
                34.0
            };
            ...
            // 2) Now take the mutable victim borrow and apply damage
            if let Some(victim) = players.get_mut(&victim_id) {
                victim.health -= damage;
                println!(
                    "[COMBAT] Player {} HP: {:.1} (damage {:.1})",
                    victim_id, victim.health, damage
                );
                ...
                if victim.health <= 0.0 {
                    ...
                    println!(
                        "[COMBAT] Player {} ELIMINATED by Player {}",
            ...
            if killed {
                kills_to_award.push(shooter_id);
            }
        }
    }
